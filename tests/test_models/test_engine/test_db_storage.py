...
class TestDBStorage(unittest.TestCase):
    ...
    def test_new(self):
        ...
        self.assertIn('max202@gmail.com', result)
        self.assertIn('qwerty', result)
        self.assertIn('Max', result)
        self.assertIn('Cephas', result)
        cursor.close()
        dbc.close()
    ...
    def test_reload(self):
        ...
        cursor.execute(
            ...
            [
                ...
                str(datetime.now()),
                str(datetime.now()),
                'dan_dickson@yahoo.com',
                'pass',
                'Daniel',
                'Dickson',
            ]
        )
        self.assertNotIn('User.3336-by-me', storage.all())
        dbc.commit()
        storage.reload()
        self.assertIn('User.3336-by-me', storage.all())
        cursor.close()
        dbc.close()
